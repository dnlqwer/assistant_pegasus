...
def play_1(core:VACore, phrase: str):
    ...
    if phrase == "отмена":
        core.say("Понял, играть не будем")
        return

    core.play_voice_assistant_speech("Не понял...")
    core.context_set(play_1)
...
def play_2(core:VACore, phrase: str):
    ...
    for i in range(1,31):
        if phrase == num2text(i):
            ...
            if i == questNumber:
                ...
                if tries >= 5:
                    txtsay += "Пять попыток прошло, к сожалению, ты проиграл. А я загадал число "+num2text(questNumber)
    ...
    core.play_voice_assistant_speech("Не понял число, скажи еще раз!")
    core.context_set(play_2)