...
def start(core:VACore):
    manifest = {
        ...
        "default_options": {
            "apiKey": "062bde93-cf1f-46ba-8942-7f58404002fd", # get at https://openweathermap.org/

            "city": "Yekaterinburg",
            "country": "Ru-ru",
            "lon": 56, # will be auto-calculated base on city. Or you can fill it manual
            "lat": 60,
        },
...
def start_with_options(core:VACore, manifest:dict):
    ...
    if options["lat"] == 0 and options["lon"] == 0: # если не указаны координаты - получаем их
        ...
        try:
            import requests
            res_str = requests.get("https://api.weather.yandex.ru/v2/forecast?lat={lat}&lon={lon}&lang={ru}",
...
def run_weather(core:VACore, phrase:str, addparam:str):
    ...
    try:
        ...
        #try:
        import requests
        res = requests.get("https://api.weather.yandex.ru/v2/forecast?lat={lat}&lon={lon}&lang={lang}",
