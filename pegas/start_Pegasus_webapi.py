...
import os
if not os.path.exists('start_Pegasus_webapi.json'):
    if os.path.exists('options/webapi.json'):
        os.rename('options/webapi.json','start_Pegasus_webapi.json')
...
def app_shutdown():
    global is_running
    cprint("Ctrl-C pressed, exiting Pegasus.", "yellow")
    is_running = False
...
if __name__ == "__main__":
    ...
    # p.start()
    if webapi_options["use_ssl"]:
        uvicorn.run("start_Pegasus_webapi:app",
