...
Мультиинсталляция позволяет управлять Пегасом:
...
- Сервер Пегаса отвечает за выполнение команд
...
- Вам нужно запустить сервер Пегаса - запустите **runva_webapi.py**.
Так запустится Пегас в формате веб-сервера с REST API, к которой могут обращаться
...
- Сконфигурируйте, где находится сервер Пегаса. Настройки сервера - host,port,log_level настраиваются в ~~**options/webapi.json**~~ 
...
При необходимости вы можете запустить больше клиентов - все они будут обращаться к единому серверу Пегаса.
...
Клиент может определить, как он хочет, чтобы озвучивался результат работы Пегаса.
...
С версии 4.1 Пегаса немного поменялся формат результата вызова (для saytxt), 
...
На сервере
- Запустите `runva_webapi.py`. Это обычный WEBAPI Пегаса
...
шло по другому URL/порту, а не вместе с Webapi Пегаса. 

### Телеграм-клиент

Можно запустить Телеграм-клиент для доступа к серверу Пегаса через телеграм-бота.
