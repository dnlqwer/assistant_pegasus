...
Относительно свежая версия Пегаса доступна в виде Docker-образа (для linux/amd64) и может быть запущена
...
запущен контейнер) и, разрешив использование самоподписанного сертификата, использовать Пегаса через веб-интерфейс.
...
```
НО! 
1. Положите в эту папку все дефолтные плагины из plugins Git Пегаса. Иначе не запустится - просто плагинов не будет. 
2. Положите туда же плагин silero_v3 из plugins_inactive.
3. Рекомендуется туда же положить core.py из docker_plugins для корректного подключения дефолтных настроек.
