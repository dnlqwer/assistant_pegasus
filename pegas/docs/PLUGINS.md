...
**plugin_greetings.py** - приветствие (оффлайн). Пример команды: "Пегас, привет"
...
При вызове команды ищется
   ...
   * или (если неудобно переназывать папку) ищется в папке файл `_pegasus.txt` в кодировке UTF-8. В нем считываются все строки, и каждая из них - это указатель имени данной папки сериала для Пегаса.
   
Например: папка "Babylon 5" содержит файл "_pegasus.txt" со строкой "вавилон". Сериал может быть запущен по фразе "сериал вавилон два" (вторая серия)
...
**plugin_gamemoreless.py** - игра Больше-меньше. Команда "игра больше меньше". Является примером работы с контекстом в движке Пегаса. 
...
Примеры команд: "Пегас акции", "Пегас акции сбер", Пегас портфель тест"

**plugin_gamemoreless_alt.py** - альтернативная реализация игры Больше-меньше. Команда "игра меньше больше". 
Является примером работы с контекстом альтернативным способом (**рекомендуется**) в движке Пегаса.
...
Примеры команд:
"Пегас скажи раз два три"
"Пегас буфер" (озвучит содержимое буфера обмена)
